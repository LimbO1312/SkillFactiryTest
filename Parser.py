...
def collect_user_rates(user_login):
    ...
    while True:
        ...
        soup = BeautifulSoup(html_content, 'html.parser')  # Исправлено
...
def get_rated_films(user_rates):
    rated_films = []
    for item in user_rates:  # Убрал цикл while True
        my_rating = item['my_rating']

        # Преобразуем строку в число с плавающей точкой, если это возможно
        try:
